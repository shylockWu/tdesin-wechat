...
let Tabs = class Tabs extends SuperComponent {
    ...
    calcScrollOffset(containerWidth, totalWidth, targetLeft, targetWidth, offset) {
    ...
    setTrack() {
        ...
        this.gettingBoundingClientRect(`.${prefix}-tabs__item`, true)
            ...
            if (this.containerWidth) {
                const offset = this.calcScrollOffset(this.containerWidth, rect.width * res.length, rect.left, rect.width, this.data.offset);
                this.setData({
                    offset,
                });
            }
